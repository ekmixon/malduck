...
def test_single_region():
    ...
    assert not list(mem.iter_regions(addr=0x10008))
    ...
    assert not list(mem.iter_regions(offset=9))

    assert not list(mem.iter_regions(length=0))
    assert list(mem.iter_regions(length=1)) == mem.regions

    assert not list(mem.iter_regions(addr=0xffff, length=1))
    ...
    assert not list(mem.iter_regions(addr=0x10008, length=0x10))
...
def test_single_region_trim():
    ...
    assert not list(mem.iter_regions(addr=0x10008, trim=True))
    ...
    assert not list(mem.iter_regions(offset=9, trim=True))

    assert not list(mem.iter_regions(length=0, trim=True))
    assert list(mem.iter_regions(length=1, trim=True)) == [Region(0x10000, 1, 0, 0, 0, 1)]

    assert not list(mem.iter_regions(addr=0xffff, length=1, trim=True))
    ...
    assert not list(mem.iter_regions(addr=0x10008, length=0x10, trim=True))
...
def test_regions_multi(mem):
    ...
    assert not list(mem.iter_regions(0x10204))
    ...
    assert not list(mem.iter_regions(offset=40))

    assert not list(mem.iter_regions(0xffff, contiguous=True))
